...
counts_path <- "C:/Users/Qiang/Desktop/SIAT/data/musWT_HD_kallisto_nobiascorrect_combined_est_counts.csv"
...
required_columns <- c("target_id", "musHD_STR-1_kallisto_est_counts", "musHD_STR-2_kallisto_est_counts", "musHD_STR-3_kallisto_est_counts", "musWT_STR-1_kallisto_est_counts", "musWT_STR-2_kallisto_est_counts", "musWT_STR-3_kallisto_est_counts")
...
selected_count <- dplyr::select(count_data, target_id,
                                `musHD_STR-1_kallisto_est_counts`,
                                `musHD_STR-2_kallisto_est_counts`,
                                `musHD_STR-3_kallisto_est_counts`,
                                `musWT_STR-1_kallisto_est_counts`,
                                `musWT_STR-2_kallisto_est_counts`,
                                `musWT_STR-3_kallisto_est_counts`)
...
rownames(count_matrix) <- selected_count$target_id  # Assuming 'Present_in_Transcripts' are the gene/feature identifiers
...
data <- data.frame(
  Sample = c("musHD_STR-1_kallisto_est_counts", "musHD_STR-2_kallisto_est_counts", "musHD_STR-3_kallisto_est_counts", "musWT_STR-1_kallisto_est_counts", "musWT_STR-2_kallisto_est_counts", "musWT_STR-3_kallisto_est_counts"),
...
ggplot(pcaData, aes(PC1, PC2, color = Type)) +
  ...
  ggtitle("PCA of musHD and musWT from Kallisto")

# Output the PCA coordinates (PC1 and PC2) to a text file
output_file <- "C:/Users/Qiang/Desktop/SIAT/data/pca_coordinates_kallisto.txt"
