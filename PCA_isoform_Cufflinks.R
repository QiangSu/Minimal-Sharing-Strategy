...
counts_path <- "C:/Users/Qiang/Desktop/SIAT/data/musWT_HD_cufflinks_nobiascorrect_combined_coverage.csv"
...
required_columns <- c("tracking_id", "musHD_STR-1_coverage", "musHD_STR-2_coverage", "musHD_STR-3_coverage", "musWT_STR-1_coverage", "musWT_STR-2_coverage", "musWT_STR-3_coverage")
...
selected_count <- dplyr::select(count_data, tracking_id,
                                `musHD_STR-1_coverage`,
                                `musHD_STR-2_coverage`,
                                `musHD_STR-3_coverage`,
                                `musWT_STR-1_coverage`,
                                `musWT_STR-2_coverage`,
                                `musWT_STR-3_coverage`)
...
rownames(count_matrix) <- selected_count$tracking_id  # Assuming 'Present_in_Transcripts' are the gene/feature identifiers
...
data <- data.frame(
  Sample = c("musHD_STR-1_coverage", "musHD_STR-2_coverage", "musHD_STR-3_coverage", "musWT_STR-1_coverage", "musWT_STR-2_coverage", "musWT_STR-3_coverage"),
...
ggplot(pcaData, aes(PC1, PC2, color = Type)) +
  ...
  ggtitle("PCA of musHD and musWT from Cufflinks")

# Output the PCA coordinates (PC1 and PC2) to a text file
output_file <- "C:/Users/Qiang/Desktop/SIAT/data/pca_coordinates_cullfinks.txt"
